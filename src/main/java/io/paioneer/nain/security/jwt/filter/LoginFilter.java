...
import io.paioneer.nain.security.jwt.model.InputUser;
...
public class LoginFilter extends UsernamePasswordAuthenticationFilter {
    private final Long accessExpiredMs; //access Token 의 만료일 (ms)
    private final Long refreshExpiredMs; //access Token 의 만료일 (ms)
    ...
    public LoginFilter (AuthenticationManager authenticationManager,
                        MemberService memberService, RefreshTokenService refreshTokenService,
                        JWTUtil jwtUtil) {
        this.jwtUtil = jwtUtil;
        this.memberService = memberService;
        this.authenticationManager = authenticationManager;
        this.refreshTokenService = refreshTokenService;
        accessExpiredMs = 1000 * 60 * 30L; //30분
        refreshExpiredMs = 1000 * 60 * 60 * 24L;   //24시간(1일)
    }
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            //이 토큰은 사용자가 제공한 이메일과 비밀번호를 담고 있으며, 이후 인증 과정에서 사용됩니다.
                    ...
                    log.info(loginData.getUsername());
...
//                    .build();
//            refreshTokenService.save(refreshToken); //db 에 저장 처리
//        }

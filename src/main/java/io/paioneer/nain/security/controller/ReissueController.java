...
public class ReissueController {
    ...
    private final RefreshTokenService refreshTokenService; // 리프레시 토큰 관련 서비스

    // 의존성 주입을 위한 생성자
    public ReissueController(JWTUtil jwtUtil, MemberService memberService, RefreshTokenService refreshTokenService) {
    ...
    //access 토큰 재발급 요청용 컨트롤러 (단, 리프래시 토큰이 정상 상태일때만 재발급 처리함)
...
//        }
        ...
        // 리프레시 토큰이 유효한지 확인합니다.
        Optional<RefreshToken> refreshTokenOptional = refreshTokenService.findByTokenValue(token);
//        if (refreshTokenOptional.isEmpty() || !refreshTokenOptional.get().getMember().toDto().equals(loginMember)) {
...
//        }
        ...
        // 리프래시 토큰이 만료되지 않은 상태이면, 새로운 액세스 토큰 생성
        String access = jwtUtil.generateToken(username, "access", accessExpiredMs / 1000);
